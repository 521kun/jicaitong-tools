(() => {
  ...
  if (location.protocol === 'file:') {
    // 双击打开时浏览器拦截 ES module，改用 UMD 版 pdf-lib
    const umd = document.createElement('script');
    umd.src = 'assets/pdf-lib.min.js';
    umd.onload = () => {
      ...
      appScript.src = 'assets/app-classic.js';
      document.head.append(appScript);
    };
    umd.onerror = () => {
      const status = document.getElementById('status');
      if (status) status.textContent = '本地组件加载失败，请保持 tools 文件夹结构完整。';
    };
    document.head.append(umd);
  } else {
    // 服务器/公网环境保持原 ES module 路径
    const moduleScript = document.createElement('script');
    moduleScript.type = 'module';
    moduleScript.textContent = [
      "import { PDFDocument, StandardFonts, rgb } from './assets/pdf-lib.esm.min.js';",
      'Object.assign(window, { PDFDocument, StandardFonts, rgb });',
      "const s = document.createElement('script');",
      "s.src = 'assets/app-classic.js';",
